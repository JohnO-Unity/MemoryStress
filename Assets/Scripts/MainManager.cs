...
public class MainManager : MonoBehaviour {
	...
	public Text textMemoryAdviceStatus;
	...
	private ulong delayMemoryAdvice = 500;	// in milliseconds
	...
	void Awake() {
		...
		textMemoryAdviceStatus.text = string.Empty;
		...
		if (errorCode == MemoryAdviceErrorCode.Ok) {
			textMemoryAdviceStatus.text = "Memory advice init successfully";
			RegisterMemAdvice();
		} else {
			textMemoryAdviceStatus.text = "Memory advice failed to init";
		}
	}

	void RegisterMemAdvice() {
		MemoryAdviceErrorCode errorCode = MemoryAdvice.RegisterWatcher(delayMemoryAdvice, new MemoryWatcherDelegateListener((MemoryState state) => {
			switch (state) {
				case MemoryState.ApproachingLimit:
					// The application should minimize memory allocation.
					textMemoryAdviceStatus.text = "<color=yellow>MemoryState.ApproachingLimit</color>";
					textMemoryAdviceStatus.Rebuild(CanvasUpdate.PreRender);
					OnPauseMemTest();
					break;
				case MemoryState.Critical:
					// The application should free memory as soon as possible
					// until the memory state changes.
					textMemoryAdviceStatus.text = "<color=yellow>MemoryState.Critical</color>";
					textMemoryAdviceStatus.Rebuild(CanvasUpdate.PreRender);
					OnPauseMemTest();
					break;
			}
		...
		if (errorCode == MemoryAdviceErrorCode.Ok) {
			textMemoryAdviceStatus.text = "Memory advice successfully registered watcher";
		} else {
			textMemoryAdviceStatus.text = "Memory advice failed to register watcher";
		}
	}
